...
import { useTopImages } from "../api/getTopImages";
...
const Index = () => {
    ...
    const { data, isLoading, isFetching } = useTopImages({
    ...
    const updateMutation = useUpdateTopImage();
    ...
    const columnHelper = createColumnHelper<TopImageType>();

    const columns = useMemo<ColumnDef<TopImageType>[]>(() => {
        const commonColumn = [];
        ...
        return [TableCheckbox<TopImageType>(columnHelper), ...commonColumn];
    ...
    const localeTableOptions = {
        ...
        enableRowSelection: (row: Row<TopImageType>) => {
